...
def create_library(number_of):
    ...
    for n in range(number_of):
        ...
        movie = Movie(title=fake.word().title(), year=fake.year(), type= type[2], watched=fake.random_int(0,20))
    ...
    def content_type(genre_type, quantity):
        ...
        if genre_type == "M":
            for n in range(quantity):
                fake = Faker()
                contact = Movie(title=fake.word().title(), year=fake.year(), type= type[2], watched=fake.random_int(0,20))
